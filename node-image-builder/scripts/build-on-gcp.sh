...
function cleanup() {
  exit_status=$?
  echo "Cleaning up..."
  # ( gcloud compute instances delete $instance_name --quiet --zone $zone --project $project_id )
...
gcloud compute scp $this_dir/setup-gcp-instance.sh builder@$instance_name:/tmp/ --zone $zone --project $project_id
gcloud compute ssh --zone $zone --project $project_id builder@$instance_name -- 'sudo /tmp/setup-gcp-instance.sh '"$PACKER_VERSION"''

echo "Syncing current directory source to the GCP instance and running: $@"
rm -rf ./.build
...
gcloud compute ssh --zone $zone --project $project_id builder@$instance_name -- 'chmod +x /tmp/build.sh'
gcloud compute ssh --zone $zone --project $project_id builder@$instance_name -- '/tmp/build.sh'
if [ -d /tmp/build-src/.build ]; then
  gcloud compute scp builder@$instance_name:/tmp/build-src/.build $(pwd)/ --recurse --zone $zone --project $project_id
fi
